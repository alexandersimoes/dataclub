...
@mod.route('/analysis/')
def analysis():
    g.subpage_type = "analysis"
    return render_template("about/analysis.html")

@mod.route('/testimonial/')
def testimonial():
    g.subpage_type = "testimonial"
    return render_template("about/testimonial.html")

@mod.route('/data/<data>/')
def data(data):
    g.subpage_type = "data"
    return render_template("about/data/index.html", data=data)

@mod.route('/glossary/<term>/')
def glossary(term):
    g.subpage_type = "glossary"
    return render_template("about/glossary/index.html", term=term)

@mod.route('/apps/<app>/')
def apps(app):
    g.subpage_type = "apps"
    return render_template("about/apps/index.html", app=app)

@mod.route('/terms_of_use/', methods=['GET'])
def terms_of_use():
    g.subpage_type = "terms_of_use"
    return render_template("about/" + gettext("terms_of_use") + ".html")

@mod.route('/classification/<attr>/<depth>/')
def attrs(attr="bra",depth="2"):

    g.subpage_type = "attrs"
    ...
    depths = [{"depth": d, "title": translate("{0}_{1}".format(attr,d))} for d in depths[attr]]

    return render_template("about/attrs.html", data_url = data_url, depths = depths, attr = attr, depth = int(depth))

@mod.route('/')
@mod.route('/contact/')
def contact():
    g.subpage_type = "contact"
    search_form = SearchForm()
    return render_template("about/ask/index.html", search_form = search_form)

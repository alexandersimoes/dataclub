...
import json
from dataviva.account.helper import add_action, add_attrview, add_pageview, get_recent
# models
# forms
# utils
# login types
...
def recent():
    ...
    uid = get_userid()
    data = get_recent(uid, limit=limit, offset=offset)
    return jsonify(data)
...
def attr_view():
    uid = get_userid()
    for attr, value in request.args.iteritems():
        for attr_id in value.split(","):
            add_attrview(uid, attr, value)
    result = {"status": "OK"}
    return jsonify(result)

@mod.route('/action/<key>')
@mod.route('/action/<key>/<value>')
def action(key, value=None):
    uid = get_userid()
    add_action(uid, key, value)
    result = {"status": "OK"}
    return jsonify(result)

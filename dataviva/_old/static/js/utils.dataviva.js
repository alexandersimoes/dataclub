...
dataviva.format.number = function(value,name,l) {
  ...
  if (!l) l = dataviva.language;

  var negative = value < 0, return_value;
  ...
  if (ids.indexOf(name) >= 0) return value.toString().toUpperCase();
    ...
    var r = value.toString().split("");
    var length = false;
    r.forEach(function(n,i){
      if (n != "0" && n != "." && !length) length = i;
    });
    if (length > 5) length = 5;
    return_value = d3.round(value,l);
    ...
    return_value = d3.format(".2f")(value);
  }
  else {
...
dataviva.ui.tooltip = function(id,obj,align) {
  if (obj) {
    ...
    if (!align) align = "bottom center";

    var t = align.split(" ")[0];
    if (t == "center") offset = size.width/2;
    else offset = size.height/2;

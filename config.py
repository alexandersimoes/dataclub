...
SQLALCHEMY_DATABASE_URI = "mysql://{0}:{1}@{2}/{3}".format(
    get_env_variable("DATAVIVA2_DB_USER", "root"),
    get_env_variable("DATAVIVA2_DB_PW", ""),
    get_env_variable("DATAVIVA2_DB_HOST", "localhost"),
    get_env_variable("DATAVIVA2_DB_NAME", "dataviva2"))
...
'''
REDIS_HOST = get_env_variable("DATAVIVA_REDIS_HOST", "localhost")
REDIS_PORT = get_env_variable("DATAVIVA_REDIS_PORT", 6379)
REDIS_PW = get_env_variable("DATAVIVA_REDIS_PW", None)
...
REDIS_CACHE = RedisCache(host=REDIS_HOST,
         port=REDIS_PORT,
         password=REDIS_PW, default_timeout=2591999, key_prefix=CACHE_PREFIX)
